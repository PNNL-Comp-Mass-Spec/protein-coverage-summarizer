...
PeptideToProteinMapper.exe /I:PeptideInputFilePath /R:ProteinInputFilePath
 ...
 [/N:InspectParameterFilePath] [/G] [/H] [/K] [/A] [/L] [/VerboseLog]
...
Use /F to specify the peptide input file format code.  Options are:
   0=Auto Determine: Treated as /F:1 unless filename ends in _inspect.txt, then treated as /F:3
   ...
   4=MSGF-DB search results file (peptide sequence in the 8th column)

When processing an Inspect search results file, use /N to specify the Inspect 
parameter file used (required for determining the mod names embedded in the identified peptides).
